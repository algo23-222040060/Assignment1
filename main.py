...
def INIT_SETTINGS(): # global variable
    global MODE
    global confidence
    global MC_n
    global MC_t
    ...
    MODE=2 #MODE 0 is testing the [rows] amount of rows. MODE 1 is testing the rest of the rows. MODE 2 is testing all rows.
    confidence = .95 #Default to 0.95
    rows=1000 #Number of rows for VaR calculation.
    plot_flag=True #To plot or not to.

    MC_n=100 #Number of samples in each Monte Carlo Method testing.
    MC_t=1000 #Number of testing in Monte Caelo simulation method.

    MC_test_flag=False
    MC_test_n=100 #Number of testing the consistency of the method
    return
...
def MC_TEST():
    ...
    for i in range(MC_test_n):
        ...
        print("Working on it... %f%%" % (i/ MC_test_n*100))
    avg=np.average(res)
    std=np.std(res)
